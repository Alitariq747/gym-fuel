...
struct SubscriptionPaywallSheet: View {
    ...
    private var selectedPackage: Package? {
        subscriptionViewModel.paywallPackages.first { $0.identifier == selectedPackageIdentifier }
            ?? subscriptionViewModel.paywallPackages.first
    }
    ...
    var body: some View {
        ...
        .task {
            ...
            if selectedPackageIdentifier == nil {
                selectedPackageIdentifier = subscriptionViewModel.paywallPackages.first?.identifier
            }
        }
    }
        ...
        VStack(spacing: 12) {
            Button {
                Task {
                    ...
                    if subscriptionViewModel.currentPlan == .pro {
                        dismiss()
                    }
                }
            } label: {
            ...
            .buttonStyle(.plain)
            .disabled(subscriptionViewModel.isLoading || subscriptionViewModel.isPurchasing)
    ...
    private var packageOptions: some View {
        VStack(spacing: 10) {
            if subscriptionViewModel.isLoading && subscriptionViewModel.paywallPackages.isEmpty {
                ProgressView()
                    .padding(.vertical, 28)
            } else {
    ...
    private func packageCard(for package: Package) -> some View {
        ...
        let isYearly = package.storeProduct.productIdentifier == SubscriptionConfig.proYearlyProductIdentifier
        let isTrialEligible = isEligibleForTrial(package)
        ...
        return Button {
            ...
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text(isTrialEligible ? "3-day free trial included" : billingText(for: package))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
    ...
    private var continueButton: some View {
        Button {
            ...
            Task {
                let didPurchase = await subscriptionViewModel.purchase(package: selectedPackage)
                if didPurchase {
                    dismiss()
                }
            }
        ...
        .buttonStyle(.plain)
        .disabled(selectedPackage == nil || subscriptionViewModel.isPurchasing || subscriptionViewModel.isLoading)
        .opacity(selectedPackage == nil ? 0.55 : 1)
    }

    private var continueButtonTitle: String {
        guard let selectedPackage else { return "Continue" }
        return isEligibleForTrial(selectedPackage) ? "Start 3-day free trial" : "Continue"
    }
    ...
    private func isEligibleForTrial(_ package: Package) -> Bool {
        ...
        return subscriptionViewModel.introEligibilityByProductIdentifier[productIdentifier] == .eligible
    }
    ...
    private func footerText(for package: Package) -> String {
        ...
        if isEligibleForTrial(package) {
            return "3-day free trial, then \(package.localizedPriceString) \(isYearly ? "per year" : "per month"). Renews automatically unless cancelled."
        }
